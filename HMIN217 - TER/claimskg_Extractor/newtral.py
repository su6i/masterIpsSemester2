...
class NewtralFactCheckingSiteExtractor(FactCheckingSiteExtractor):
    ...
    def retrieve_urls(self, parsed_listing_page: BeautifulSoup, listing_page_url: str, number_of_pages: int) \
        ...
        json_output = caching.get(query_url.format(
            offset=0), headers=self.headers, timeout=5)
        offset = 0
        ...
        while json_output.strip() != "[]":
            ...
            offset += 100
            json_output = caching.get(query_url.format(
                offset=offset), headers=self.headers, timeout=5)
        return urls
...
NewtralFactCheckingSiteExtractor.extract_claim_and_review()
